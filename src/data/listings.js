...
const DEMO_STAY_LISTINGS = __stayListing.map((post, index) => {
...
const DEMO_EXPERIENCES_LISTINGS = __experiencesListing.map((post, index) => {
  //  ##########  GET CATEGORY BY CAT ID ######## //
...
const DEMO_CAR_LISTINGS = __carsListing.map((post, index) => {
  //  ##########  GET CATEGORY BY CAT ID ######## //
  const category = DEMO_EXPERIENCES_CATEGORIES.filter(
    (taxonomy) => taxonomy.id === post.listingCategoryId
  )[0];
